...
export const fakeFieldResolver: GraphQLFieldResolver<unknown, unknown> = async (
  ...
  function fakeValueOfType(type) {
    ...
    if (isLeafType(type)) {
      ...
      return fakeLeafValueCB(type, fieldName, randomSeed);
    } else {
  ...
  function getListLength(object): number {
...
function fakeLeafValueCB(
  ...
  // 如果匹配到 smart faker ，直接返回
  const smartFaker = smartFakeValue(fieldName, type, randomSeed);
  // const smartFaker = fakeValue(fieldName, {
  //   randomSeed,
  // });
  if (smartFaker) return smartFaker;
